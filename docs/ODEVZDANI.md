...
| ------------------- | ---------------------- | ----------- |
| Student A – vedoucí | Datový model           | 1           |
| Student B           | WebAPI & SSE & Backend | 1           |
| Student C           | Blazor klient & UI     | 1           |
| Student D           | Testovani              | 1           |
...
2. Spustit aplikaci s předpřipravenými daty (Keycloak realm a uživatelé, popř. SQL data), použijte předimportované Docker volume podle návodu v **docs/DOCKER-VOLUMES.md** (export volume na zdrojovém stroji, přenos archivu, vytvoření volume a rozbalení na cílovém stroji před prvním startem).
3. Otevřete solution **UTB.Minute.sln** ve JetBrains Rider.
4. Nastavte projekt **UTB.Minute.AppHost** jako **Start-up projekt**.
5. Spusťte projekt.
6. V prohlížeči se otevře **.NET Aspire Dashboard**, kde uvidíte stav všech služeb a odkazy na klientské aplikace (AdminClient, CanteenClient) a WebApi.
